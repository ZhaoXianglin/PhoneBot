<template>
  <div class="posttest">
    ...
    <van-form @submit="onSubmit" scroll-to-error @failed="onFailed" validate-trigger="onSubmit">
      <p style="padding:0 10px;font-weight:bold;text-align: left">1. What else do you want to comment on the recommendations provided by the chatbot?
      </p>
      <van-field
          v-model="openended1"
          name="openended1"
      ...
      />
      <p style="padding:0 10px;font-weight:bold;text-align: left">2. What else do you want to comment on the interaction with the chatbot?</p>
      <van-field
          name="openended2"
          v-model="openended2"
          rows="4"
